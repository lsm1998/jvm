package com.lsm1998.jvm.vm.clazz.attribute.impl;

import com.lsm1998.jvm.vm.clazz.ClassRead;
import com.lsm1998.jvm.vm.clazz.attribute.AttributeInfo;
import com.lsm1998.jvm.util.ClassReadUtil;

/**
 * @作者：刘时明
 * @时间：2019/3/15-20:27
 * @说明：
 */
public class LineNumberTable implements AttributeInfo
{
    // 属性名称在常量池的索引（u2类型）
    public int attributeNameIndex;
    // 属性长度（u4类型）
    public long attributeLength;
    // 行号属性表长度
    public int lineNumberTableLength;
    // 行号属性表
    public LineNumberInfo[] lineNumbers;

    @Override
    public void readInfo(ClassRead classRead)
    {
        attributeNameIndex= ClassReadUtil.readU2(classRead);
        attributeLength=ClassReadUtil.readU4(classRead);
        lineNumberTableLength= ClassReadUtil.readU2(classRead);
        lineNumbers=new LineNumberInfo[lineNumberTableLength];
        for (int i = 0; i < lineNumberTableLength; i++)
        {
            lineNumbers[i]=new LineNumberInfo();
            lineNumbers[i].readInfo(classRead);
        }
    }
}
